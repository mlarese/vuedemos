...
export default class VueElementQuery {
   ...
   */
  constructor (ResizeSensor, element,state) {
        ...
        if(!this.state.queryElements){
            Vue.set(this.state,'queryElements',{});
        }
        this.init(element);
  }
  ...
  watchElement (element) {
      let setValues =()=>{
        ...
        let id= element.id||element.classList[0],
            maxWidth=element.clientWidth,
            minWidth=element.clientWidth;
        Vue.set( this.state.queryElements,id,{id,maxWidth,minWidth });
    };

    new this.ResizeSensor(element, _debounce(setValues,20) );
  }
  ...
  crawlRules (rules, callback) {
    for (let i = 0, j = rules.length; i < j; i++) {
      if (rules && rules[i] && rules[i].selectorText && callback)
  ...
  getValidSelector (selectorText, callback) {
    ...
    matches.forEach(selector => {
      if (selectorText.indexOf(selector) !== -1) {
        ...
        let match = selectorText.match(regex);
        let mv = 0;
